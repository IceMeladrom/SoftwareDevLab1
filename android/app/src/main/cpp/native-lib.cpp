...
JNICALL
Java_ru_bmstu_iu3_lab1_MainActivity_initRng(JNIEnv *env, jclass clazz) {
...
JNICALL
Java_ru_bmstu_iu3_lab1_MainActivity_randomBytes(JNIEnv *env, jclass, jint no) {
...
JNICALL
Java_ru_bmstu_iu3_lab1_MainActivity_encrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data) {
...
JNICALL
Java_ru_bmstu_iu3_lab1_MainActivity_decrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data) {
    jsize ksz = env->GetArrayLength(key);
    jsize dsz = env->GetArrayLength(data);
...
JNICALL JNI_OnLoad(JavaVM *pjvm, void *reserved) {
    gJvm = pjvm;
...
Java_ru_bmstu_iu3_lab1_MainActivity_transaction(JNIEnv *xenv, jobject xthiz, jbyteArray xtrd) {
    ...
    std::thread t([thiz, trd] {
        ...
        // TRD: 9F0206000000000100 = сумма 1.00р в формате BCD
        uint8_t *p = (uint8_t *) env->GetByteArrayElements(trd, 0);
        jsize sz = env->GetArrayLength(trd);
        if ((sz != 9) || (p[0] != 0x9F) || (p[1] != 0x02) || (p[2] != 0x06))
            return false;  // Если данные транзакции некорректны, выходим из потока.
        char buf[13];
        for (int i = 0; i < 6; i++) {
            uint8_t n = *(p + 3 + i);
            buf[i * 2] = ((n & 0xF0) >> 4) + '0';
        ...
        while (ptc > 0) {
            ...
            // Освобождаем строку после получения её содержимого.
        ...
        jmethodID tid = env->GetMethodID(cls, "transactionResult", "(Z)V");
        env->CallVoidMethod(thiz, tid, ptc > 0);
